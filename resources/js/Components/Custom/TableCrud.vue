...
const form = useForm<Record<string, any>>(props.formFields
...
function doDelete() {
  ...
  Inertia.delete(
    route(`${props.resourceName}.destroy`, deletingId.value),
    {
      onSuccess: () => {
      ...
      <div class="bg-white rounded-lg p-6 w-full max-w-lg relative">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div v-for="f in formFields" :key="f.key">
            ...
            />
            <div v-if="form.errors[f.key]" class="text-red-600 text-sm">
              {{ form.errors[f.key] }}
            </div>
          </div>
